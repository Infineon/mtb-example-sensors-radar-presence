...
static void terminal_ui_menu(void)
{
    ...
    result = mtb_radar_sensing_get_parameter(&sensing_context,
    ...
    {
        printf("ERROR: mtb_radar_sensing_get_parameter() returned an error while reading 'radar_presence_range_max'");
    }
    else
    {
    ...
    if (result != CY_RSLT_SUCCESS)
    {
        printf("ERROR: mtb_radar_sensing_get_parameter() returned an error while reading 'radar_presence_sensitivity'");
    }
    else
    {
...
static void terminal_ui_readline(void *uart_ptr, char *line, int maxlength)
{
    ...
    while ((CY_RSLT_SUCCESS == result) && (rx_value != '\r') && (--maxlength > 0))
    {
        result = cyhal_uart_getc(uart_ptr, &rx_value, 0);
        /* Echo character */
        if (CY_RSLT_SUCCESS == result)
        {
            (void)cyhal_uart_putc(uart_ptr, rx_value);
